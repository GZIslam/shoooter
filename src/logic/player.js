...
const createPerson = ({ name, health, canShoot, size, hearts, speed, damage,
    ...
    const player = {
        name, health, canShoot, size, hearts, speed, damage, position, type,
        uuid, cooldown, score: 0,
        fullHealth: health,
        animate: function () {
            if (logic)
                logic(this, world);
            position.x = (position.x + this.speed.x > world.mapSize.w - player.size) || (position.x + this.speed.x < 0 + player.size / 2) ? position.x : position.x + this.speed.x;
            position.y = (position.y + this.speed.y > world.mapSize.h - player.size) || (position.y + this.speed.y < 0 + player.size / 2) ? position.y : position.y + this.speed.y;
        },
        shoot: function (destination) {
            if (cooldownReady()) {
                ...
                world.bullets.push(createBullet({
                    damage, size: 3, speed: bSpeed, owner: this,
...
export const createPlayer = (world, name) => {
    const player = createPerson({
        ...
        damage: 1,
        position: { x: world.mapSize.w / 2, y: world.mapSize.h / 2 },
        bulletSpeed: 10,
        cooldown: 0.1,
    }, world);
    // controller(player);
...
export const createEnemy = (props, world) => {
    ...
    return createPerson({
        type: 'enemy',
        hearts: 1,
        position: { x: Math.random() * world.mapSize.w, y: Math.random() * world.mapSize.h },
        logic,
        ...props
    }, world);
}
